...
DEFAULT_TESTSET = {
    "zad1": {
        "cases": [
            {"inp": "black g8 h1 c4", "out": 10},
            {"inp": "black b4 f3 e8\n", "out": "6\n"},
            {"inp": "white a1 e3 b7", "out": 9},
            {"inp": "black h7 a2 f2", "out": 6},
            {"inp": "black a2 e4 a4", "out": 8},
        ],
        "defaults": {"input_file": "zad1_input.txt", "output_file": "zad1_output.txt", "timeout": 10},
    ...
    "zad2": {
        ...
        "defaults": {"input_file": "zad2_input.txt", "output_file": "zad2_output.txt", "timeout": 60},
    ...
    "zad4": {
        ...
        "defaults": {"input_file": "zad4_input.txt", "output_file": "zad4_output.txt", "timeout": 10},
    ...
    "zad5": {
        ...
        "defaults": {"input_file": "zad5_input.txt", "output_file": "zad5_output.txt", "timeout": 30},
        "validator": "perlines_validator",
    },
}
...
def load_word_list():
    ...
    zad2_words = set()
    with gzip.open("data/words_for_ai1.txt.gz", "rb") as gf:
        for line in gf:
            line = line.strip()
            if line:
...
def zad2_line_compare(returned, expected, message="Contents"):
    ret = compare(returned.replace(" ", ""), expected.replace(" ", ""), message + " string without spaces differs!")
    if ret:
        return ret
...
def compare(returned, expected, message="Contents"):
    if returned != expected:
        return '%s differ. Got: "%s", expceted: "%s"' % (message, returned, expected)
    return None
...
def perlines_validator(case, process_out, line_compare_fun=compare):
    ...
    if ret:
        return ret
    for lnum, (proc_line, ref_line) in enumerate(zip(process_lines, ref_lines)):
        ret = line_compare_fun(proc_line, ref_line, "Line %d contents" % (lnum + 1,))
        if ret:
            return ret
    return None
...
def run_case(program, inp, out=None, input_file="<stdin>", output_file="<stdout>", timeout=1.0):
    del out  # unused
    ...
    try:
        ...
        process = subprocess.Popen(program, shell=True, stdin=stdin, stdout=stdout, **kwargs)
        if timeout > 0:
...
def get_argparser():
    ...
    parser.add_argument("--cases", default="", help="Comma-separated list of test cases to run, e.g. 1,2,3-6.")
    parser.add_argument("--show_example", default=False, action="store_true", help="Print a sample input/output pair.")
    parser.add_argument(
        "problem", help="Problem form this homework, one of: %s." % (", ".join(sorted(DEFAULT_TESTSET.keys())),)
    )
    parser.add_argument("program", nargs=argparse.REMAINDER, help="Program to execute, e.g. python solution.py.")
    return parser
...
if __name__ == "__main__":
    ...
    if args.problem not in testset:
        print("Problem not known: %s. Choose one of %s." % (args.problem, ", ".join(sorted(testset.keys()))))
    ...
    for case_num, case_def in problem_cases:
        ...
        case_ret = run_and_score_case(program, problem_def["defaults"], case_def, problem_validator)
        if case_ret is None:
            print("OK!")
        else:
    ...
    if failed_cases:
        print("\nSome test cases have failed. " "To rerun the failing cases execute:")
        testset_opt = ""
        cases_opt = "--cases " + ",".join([str(fc) for fc in failed_cases])
        print("python validator.py%s %s %s %s" % (testset_opt, cases_opt, args.problem, program))
