...
def kill_proc(process):
    if process.poll() is None:
        print("Killing subprocess:", process)
...
def play(
    ...
    results = []
    timeout_result = {0: -1000, 1: 1000}  # map players to results after a timeout
    ...
    def play_game():
        ...
        try:
            ...
            player_remaining_time = max(0, game_timeout - p[cur_player].total_time)
            ...
            if VERBOSE:
                game.draw()
            while True:
                move = p[cur_player].expect("IDO", timeout=player_move_time)
                if start_player == 0:
                ...
                timeout_winner = oponent
                player_remaining_time = game_timeout - p[cur_player].total_time
                if player_remaining_time <= 0:
                    print(p[cur_player], "px ran out of time")
                    raise queue.Empty
                player_move_time = min(move_timeout, player_remaining_time)
                p[cur_player].put(
                    "HEDID %f %f %s" % (player_move_time, player_remaining_time, move),
                    measure_time=True,
                )
        except WrongMove:
            if VERBOSE > 1:
...
if __name__ == "__main__":
    games = {"reversi": Reversi, "jungle": Jungle, "chess": Chess}
    parser = argparse.ArgumentParser()
    parser.add_argument("-v", "--verbose", default=0, type=int)
    parser.add_argument("-n", "--num_games", default=10, type=int)
