...
class Sokoban(object):
    ...
    def map_to_string(empty_map, state):
        ...
        if empty_map[k_loc[0], k_loc[1]] == Sokoban.GOAL:
            strings[k_loc[0]][k_loc[1]] = Sokoban.id2char[Sokoban.KEEPER_ON_GOAL]
        else:
...
def sokoban_validator(case, process_out, message=""):
    ...
    state = Sokoban.moves_to_strings(empty_map, state, k_moves)
...
def komandos_validator(case, process_out, message=""):
    ...
    if solved_fraction == 1:
        ...
        fail("%sLevel solved in %f%% only!" % (message, solved_fraction * 100.0))
...
def compare(returned, expected, message="Contents"):
    if returned != expected:
        fail('%s differ. Got: "%s", expceted: "%s"' % (message, returned, expected))
...
def run_case(
    program, inp, out=None, input_file="<stdin>", output_file="<stdout>", timeout=1.0
):
    del out  # unused
...
def get_argparser():
    ...
    parser.add_argument(
        "--testset", default="", help="Path to a YAML test set definition."
    )
    parser.add_argument(
        "--show_example",
        default=False,
...
def start_benchmark():
    ...
    result = timeit.repeat(
        "validator2.simple_benchmark()", setup="import validator2", number=10, repeat=10
    )
...
if __name__ == "__main__":
    ...
    if args.problem not in testset:
        print(
            "Problem not known: %s. Choose one of %s."
    ...
    for case_num, case_def in problem_cases:
        ...
        try:
            timeout_multiplier = (
                float(args.timeout_multiplier)
                if args.timeout_multiplier and float(args.timeout_multiplier) > 1
                else 1
            )
            if args.stdio:
            ...
            case_meas = run_and_score_case(
                program,
                problem_def["defaults"],
                case_def,
                problem_validator,
                timeout_multiplier * TIME_MULTIPLIER,
            )
