...
def flatten(lst: typing.Iterable[typing.Iterable[typing.Any]]
...
def read_input(filename="zad_input.txt") -> Board_Metadata:
    with open(filename, "r") as f:
        data = f.read().splitlines()
    size_str, rest_str = data[0], data[1:]
    size = tuple(map(int, size_str.split()))
    rest = tuple(tuple(map(int, row.split())) for row in rest_str)
...
def sum_permutations(length: int, total_sum: int) -> typing.Generator[tuple[int], None, None]:
    if length == 1:
        yield (total_sum,)
    else:
