...
class Board:
    ...
    def __init__(self, init_str: str = None, init_board_pos: int = None) -> None:
    ...
    def change_snapshot(snapshot: int, piece: Piece, target: int) -> int:
        if piece == 5:  # Piece.make_piece(Piece.White | Piece.King):
            shift = 12
        elif piece == 7:  # Piece.make_piece(Piece.White | Piece.Rook):
            shift = 6
        elif piece == 9:  # Piece.make_piece(Piece.Black | Piece.King):
            shift = 0
        else:
    ...
    def make_snapshot(self) -> int:
        ...
        return (self.color_to_move << 18) + (self.white_king << 12) + (self.white_rook << 6) + self.black_king
    ...
    def generate_moves_white_king(self):
        ...
        rmoves = {self.black_king + move for move in KING_MOVES[self.black_king]} | {self.black_king, self.white_rook}
        return [move for move in moves if move not in rmoves]

    def gen_all_moves(self):
        return self.generate_moves_black_king() + self.generate_moves_white_king() + self.generate_moves_white_rook()

    def generate_future_states(self):
        make_states = lambda piece, moves: deque(Board.change_snapshot(cur, piece, move) for move in moves)
...
def solve(input_str, debug_mode=False) -> str:
    ...
    while to_visit:
        ...
        if current_board.eval_check() and not future_states:
            king_prox = [mv + current_board.black_king for mv in KING_MOVES[current_board.black_king]]
            if current_board.white_rook not in king_prox:
...
def main(validator_mode: bool = False):
    if not validator_mode:
        ...
        with open("zad1_input.txt", "r") as f:
            ...
            sol = len(solve(inp).split())
            print(sol)
            with open("zad1_output.txt", "w") as of:
                of.write(str(sol))


# To debug: print("Debug messages...", file=sys.stderr, flush=True)
print(solve("white a1 e3 b7", debug_mode=True))
# col, wk, wr, bk = init_str.split()
main(True)
